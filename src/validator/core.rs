...
pub(crate) struct Module {
    ...
    pub imports: IndexMap<(String, String), Vec<EntityType>, RandomState>,
...
impl Module {
    ...
    pub(crate) fn imports_for_module_type(
        &self,
        offset: usize,
    ) -> Result<IndexMap<(String, String), EntityType, RandomState>> {
...
mod arc {
    ...
    impl<T> MaybeOwned<T> {
        ...
        fn make_shared(&mut self) {
            ...
            let inner = crate::std::mem::replace(&mut self.inner, Inner::Empty);
            let x = match inner {
                Inner::Owned(x) => x,
                _ => Self::unreachable(),
            };
