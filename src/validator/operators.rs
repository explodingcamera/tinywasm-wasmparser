...
const _: () = {
    assert!(crate::std::mem::size_of::<MaybeType>() == 4);
};

impl crate::std::fmt::Display for MaybeType {
    fn fmt(&self, f: &mut crate::std::fmt::Formatter<'_>) -> crate::std::fmt::Result {
        match self {
            ...
            MaybeType::Type(ty) => crate::std::fmt::Display::fmt(ty, f),
        }
    }
}
...
trait PreciseIterator:
    ExactSizeIterator + DoubleEndedIterator + Clone + crate::std::fmt::Debug
{
}
impl<T: ExactSizeIterator + DoubleEndedIterator + Clone + crate::std::fmt::Debug> PreciseIterator
    for T
{
}
