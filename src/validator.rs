...
impl Validator {
    ...
    pub fn end(&mut self, offset: usize) -> Result<Types> {
        match crate::std::mem::replace(&mut self.state, State::End) {
...
mod tests {
    ...
    fn test_module_type_information() -> Result<()> {
        ...
        let types = validator.validate_all(&bytes).unwrap();
    ...
    fn test_type_id_aliasing() -> Result<()> {
        ...
        assert!(crate::std::ptr::eq(&types[t_id], &types[a1_id],));
        assert!(crate::std::ptr::eq(&types[t_id], &types[a2_id],));
